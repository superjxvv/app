...
function test(id = _id) {
  ...
  //}
  API.graphql(
    graphqlOperation(mutations.updateDevice, {
      input: {
        id: id,
...
function testBlast(nextToken, startTime = Date.now()) {
  API.graphql(graphqlOperation(queries.listDevices, { nextToken })).then(
    value => {
...
function cascade(id, now) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const ratio = 0.5 + 0.5 * Math.sin((2 * Math.PI * i) / 6 + id / 10);
...
function updateCanvas() {
  ...
  if (Color.points) {
    for (let i = 0; i < Color.points.length; i++) {
      ...
      let p1 =
        i < Color.points.length - 1 ? Color.points[i + 1] : Color.points[i];
      if (p1.time < t) {
        // Past
        Color.points.shift();
        i--;
        ...
        const current = chroma.mix(
          chroma(p.color._rgb),
          chroma(p1.color._rgb),
          ratio,
          "hsl"
        );
...
function createDevice(id, seat) {
  API.graphql(
    graphqlOperation(mutations.createDevice, { input: { id, seat } })
  ).then(console.log);
}
